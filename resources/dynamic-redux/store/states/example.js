...
const exampleState = new DynamicState(
  // name of the state
  'example',
  // initial state
  {
  ...
  // state's options
  {
    // set `strictTyping` to true. Which will make the state throw an error if a property change of type
    strictTyping: true,
  }
);
...
exampleState.createActions({
  ...
  myObject: [
    ...
    'merge',
    // 'query' grants the possibility to perform one or more complex actions on the property (can also be set on arrays)
    'query'
  ],
  ...
  // with the action set to trigger it make the possibility for
  // `myAction` to be an action without having its own property in the state
  // which means this action won't modify the state on its own
  myAction: 'trigger',
...
state.createSelectors({
  numberOfPropertiesInMyObject: state => Object.keys(state.myObject).length,
});

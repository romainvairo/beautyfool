...
export class DynamicState {
  ...
  dispatcher = (state, action, prop) => {
    switch (action.kind) {
      ...
      case 'trigger':
        break;
      case 'query':
        this.setter(state, prop, action.payload(new Query(state[prop], action)).result);
        break;

      default:
        throw new Error(`There is no action kind "${action.kind}"`);
    }
  }
  ...
  actionCreator = action => {
    ...
    return (value, dispatch) => {
      ...
      const actionObject = { type, payload: value, kind: action.kind, prop: action.prop };
  ...
  createActions(_actions) {
    ...
    _.forIn(_actions, (action, actionName) => {
      ...
      const isTrigger = action === 'trigger' || action.includes('trigger');
